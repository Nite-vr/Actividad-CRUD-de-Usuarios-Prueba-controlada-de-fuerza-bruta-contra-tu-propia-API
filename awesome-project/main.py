...
DataBase: List[User] = [
    User(id=1, username="Nicolas", password="Uide", email="nicolas@gmail.com", is_active=True),
    User(id=2, username="Lily", password="132", email="lily@gmail.com", is_active=False),
]
...
def get_user(id: int):
    for usuario in DataBase:
        if usuario.id == id:
            return usuario
    raise HTTPException(status_code=404, detail="Ese bro no esta aqui")
...
def update_user(id: int, user_update: User):
    for i, user in enumerate(DataBase):
        if user.id == id:
            ...
            DataBase[i] = user
            return user
    raise HTTPException(status_code=404, detail="Ese bro no esta aqui x2")
...
def delete_user(id: int):
    for usuario in DataBase:
        if usuario.id == id:
            ...
            return usuario          
    raise HTTPException(status_code=404, detail="Ese bro no esta aqui x3")
...
def login(request: Login):
    ...
    raise HTTPException(status_code=401, detail="Ese bro no esta aqui x4")